...
for Z in range(len(patient_list)): 
    ...
    if average_cortical == 0: 
            ...
            saving_name = 'decoding_2mvts_individual_trials_C_contra_decimate_window2_perpatient_shuffled'
            saving_name_bis = 'decoding_2mvts_individual_trials_C_contra_decimate_window2_perpatient_shuffled_training_scores'
            ...
            target_shuf = np.random.permutation(target)# Shuffling        
            ...
            for n in range(number_seeds):
                ...
                for train_index, test_index in kfold.split(Full_data,target):
                    ...
                    y_train,y_test = target_shuf[train_index],target_shuf[test_index]
        ...
        saving_name = 'decoding_2mvts_permutation_trials_Fp_F_C_P_contra_decimate_window2_perpatient_shuffled' 
        saving_name_bis = 'decoding_2mvts_permutation_trials_Fp_F_C_P_contra_decimate_window2_perpatient_shuffled_training_scores' 
        ...
        for v in range(number_permutations):
            ...
            Full_data = np.concatenate((EEG_simple_1_bis,EEG_simple_2_bis)); #,EEG_simple_3_bis));
            target = np.concatenate((np.ones((minimal_number,1)),2*np.ones((minimal_number,1)) )); #,3*np.ones((minimal_number,1)) ));
            target_shuf = np.random.permutation(target)
