...
for Z in range(len(patient_list)): 
    ...
    electrodes=[0,1,2] ## For EMG in jittered conditions
    
    if average_cortical == 0: 
        
            os.chdir('/Volumes/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number) #+'/'+condition)  
            extension_name='jittered_full_cortical_ERP_filtered'
            name_data='jittered_full_cortical_ERP_filtered_artefact_free_50'
            saving_name = 'decoding_2mvts_individual_trials_EMG_decimate_window2_jittered_50ms_perpatient'
            ...
            decimate_factor = 3; temporal_window = np.arange(504,1224);
            ...
            os.chdir('/Volumes/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number+'/Decodage') 
        ...
        os.chdir('/Volumes/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number+'/Average_Permutations_Jittered')  
        extension_name='average_cortical_EMG_based_ERP_filtered_jittered'
        name_data='average_cortical_ERP_filtered_jittered_200'
        ...
        for v in range(number_permutations):
            ...
            decimate_factor = 3; temporal_window = np.arange(504,1224); #temporal_window = np.arange(304,1024);
            EEG_simple_1_bis= np.zeros((minimal_number,len(electrodes),int(len(temporal_window)/decimate_factor)))
            for k in range(minimal_number):
                u=0;
                for n in electrodes:
