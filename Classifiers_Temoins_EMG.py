...
for Z in range(len(patient_list)): 
    ...
    if average_cortical == 0: 
            ...
            saving_name = 'decoding_2mvts_individual_trials_EMG_decimate_window_no_jitter_perpatient'
            ...
            decimate_factor = 3; temporal_window = np.arange(504,1224); # 600 ms including the movement EMG
        ...
        os.chdir('/Volumes/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number+'/Average_Permutations')  
        ...
        for v in range(number_permutations):
            ...
            decimate_factor = 3; temporal_window = np.arange(504,1224); #temporal_window = np.arange(910,1524);
            EEG_simple_1_bis= np.zeros((minimal_number,len(electrodes),int(len(temporal_window)/decimate_factor)))
            for k in range(minimal_number):
                u=0;
                for n in electrodes:
