...
for Z in range(7,len(patient_list)): 
    ...
    if average_cortical ==1:        

        os.chdir('/Volumes/DBS/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number+'/Average_Permutations_Frequency')  
        extension_name='average_frequency_envelopes'
        name_data='frequency_envelopes'
        saving_name = 'decoding_2mvts_permutation_trials_Fp_F_C_P_contra_delta_band_decimate_window2_perpatient' 
        saving_name_bis = 'decoding_2mvts_permutation_trials_Fp_F_C_P_contra_delta_band_decimate_window2_perpatient_training_scores' 
        ...
        for v in range(number_permutations):
            ...
            mat_1 = scipy.io.loadmat(patient_number+'_index_'+extension_name+'_'+str(v+1)+'.mat')  
            EEG_simple_1 = mat_1[name_data];
            EEG_simple_1 =EEG_simple_1['delta'][0][0]
            
            mat_2 = scipy.io.loadmat(patient_number+'_poing_'+extension_name+'_'+str(v+1)+'.mat')  
            EEG_simple_2 = mat_2[name_data];
            EEG_simple_2 =EEG_simple_2['delta'][0][0]
            
            #mat_3 = scipy.io.loadmat(condition_name+'_marionnette_'+extension_name+'_'+str(v+1)+'.mat')   
            #EEG_simple_3 = mat_3[name_data];
            #EEG_simple_3 =EEG_simple_3['beta'][0][0]
            
            minimal_number=minimal_permutation_per_patient[Z]; 
                
                 
            decimate_factor = 3; temporal_window = np.arange(304,1024);
        ...
        #os.mkdir('/Volumes/DBS/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number+'/Decodage_frequency_band')
        os.chdir('/Volumes/DBS/DBS/EEG_Analysis/DATA_EEG/TEMOINS/'+patient_number+'/Decodage_frequency_band')
